...
public class Memory {
     ...
     * 页表
     * 本质是PageItem数组
     * 物理页框号 char[] pageFrame;
     * 装入位 boolean isInMem = false;
     * size == Disk.DISK_SIZE_B / Memory.PAGE_SIZE_B 14位
     */
    private final PageItem[] pageTbl = new PageItem[Disk.DISK_SIZE_B / Memory.PAGE_SIZE_B]; // 页表

    /**
     * true 表示已被占用 false表示空闲
     * size == MEM_SIZE_B / PAGE_SIZE_B 12位
     */
    ...
    public void real_load(String pAddr, int len) {
        // TODO
        byte[] tmp = disk.read(pAddr,len);
        Memory m = Memory.getMemory();
        m.write(pAddr, len, tmp);
    }

    /**
     * 段式存储模式下，内存从磁盘加载该段索引的数据 从磁盘中加载数据.段页式存储中，不用从磁盘中加载数据
     *
     * @param segIndex 段索引
     */
    public void seg_load(int segIndex) {
        ...
        if (SEGMENT && !PAGE) {
            //分段式：
            StringBuilder paddr = new StringBuilder(Transformer.binaryToInt(String.valueOf(segIndex)));
            Memory mem = Memory.getMemory();
            //未开启分页时粒度为false
            byte[] tmp = new byte[1024 * 1024];
            //使用段基址作为访问磁盘的地址，用段限长（即段大小）作为读取的长度
            tmp = disk.read("00000000000000000000000000000000", 1024 * 1024);
            //默认把数据放在物理地址为0的地方
            mem.write("00000000000000000000000000000000", 1024 * 1024, tmp);

            //填写GDT
            char[] basetmp = getBaseOfSegDes(segIndex);
            for(int i = 0; i < 32; i++) {
                basetmp[i] = '0';
            }
            char[] limittmp = getLimitOfSegDes(segIndex);
            for (int i = 0; i < 20; i++) {
                limittmp[i] = '1';
            }
            getSegDescriptor(segIndex).granularity = false;
            ...
            getSegDescriptor(segIndex).granularity = true;
            getSegDescriptor(segIndex).validBit =true;
        }
    }
    ...
    public void page_load(int vPageNo) {
        // TODO
        String vPageAddr32 = Transformer.intToBinary(String.valueOf(vPageNo));

        StringBuilder vPageAddr = new StringBuilder(32);
        vPageAddr = new StringBuilder(vPageAddr32.substring(12));
        for (int i = 0; i < 12; i++) {
            vPageAddr.append('0');
        }
        ...
        //使用该虚页的起始地址作为访问磁盘的地址，用页大小作为读取的长度。
        tmp = disk.read(vPageAddr.toString(),  4 * 1024);

        //找出一个空闲的内存中的物理页框然后放下去
        /**
         * i 为页框号 0 ~ pow(2,12)
         * pageFrameAddr 为内存中物理地址 低24位有效
         */
        int pageFrameAddr = -1;
         ...
         * tmpstr32 为主存物理地址
         */
        String tmpstr32 = Transformer.intToBinary(String.valueOf(pageFrameAddr));

        mem.write(tmpstr32.toString(), 4 * 1024, tmp);
        //填好页表
        //已缓存
        getPageItem(vPageNo).isInMem = true;
        //存入页在主存中的地址：页框地址20位
        char[] yebiao = new char[20];
        for (int i = 0; i < 20; i++) {
            yebiao[i] = tmpstr32.charAt(i);
        }

        getPageItem(vPageNo).pageFrame = yebiao.clone();
